...
### [P24] スキル変更時のバージョンバンプ運用の確立

**As** a stakeholder, **I want** スキルに変更を加えた際にバージョンが確実にインクリメントされる, **so that** `/scrum update` のバージョン検出が正しく機能し、インストール先プロジェクトが最新の変更を取り込める。

**Acceptance Criteria:**
- [ ] 現在の SKILL.md metadata.version が実際の変更履歴と整合している（必要なら今回バンプする）
- [ ] スキルファイル変更時にバージョンバンプを行うルールが DoD または開発ルールに明記されている
- [ ] バージョニング方針（何が patch/minor/major か）が記録されている

**Notes:** Sprint 10 までエージェント定義やセレモニー定義に変更を加えてきたが、バージョンは 1.1.0 のまま。これでは `/scrum update` でバージョン比較しても差分が検出されず、インストール先に変更が反映されない。

**Dependencies:** P23（update 検証）と関連するが、独立して着手可能。P23 が「update が動くか」の検証、P24 が「バージョンを上げる運用」の確立。
